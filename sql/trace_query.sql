...
LEFT JOIN (
    SELECT ROUTE_ID,
           MAX(TRACTOR_IDENTIFIER) AS TRACTOR_IDENTIFIER,
           MAX(DRIVER1_NAME)       AS DRIVER1_NAME
    FROM DRIVER_ASSIGNMENTS
    WHERE IS_DELETED = FALSE
    GROUP BY ROUTE_ID
) da ON tu.ROUTE_ID = da.ROUTE_ID
LEFT JOIN (
    SELECT ROUTE_ID,
...
QUALIFY ROW_NUMBER() OVER (
    PARTITION BY tu.UPDATE_ID
    ORDER BY rs.SEQUENCE ASC NULLS LAST
) = 1
