...
def test_watermark_set_overwrites() -> None:
    store = InMemoryWatermarkStore()
    t1 = datetime(2026, 4, 2, 12, 0, 0, tzinfo=UTC)
    t2 = datetime(2026, 4, 2, 13, 0, 0, tzinfo=UTC)
    store.set(
        WatermarkRecord(
            pipeline="trace", last_synced_at_utc=t1, last_run_status="success", last_run_at_utc=t1
        )
    )
    store.set(
        WatermarkRecord(
            pipeline="trace", last_synced_at_utc=t2, last_run_status="success", last_run_at_utc=t2
        )
    )
...
def test_watermark_pipelines_are_independent() -> None:
    ...
    store.set(
        WatermarkRecord(
            pipeline="trace", last_synced_at_utc=now, last_run_status="success", last_run_at_utc=now
        )
    )
...
def test_audit_delete_older_than_filters_by_pipeline_and_cutoff() -> None:
    ...
    new = datetime(2026, 4, 2, tzinfo=UTC)
